...
const run = async (fileName) => {
	...
	for (let y = 0; y < topoMap.length; y++) {
		for (let x = 0; x < topoMap[y].length; x++) {
			if (topoMap[y][x] === "0") {
				...
				const walkDownPath = (currentPos: [number, number]) => {
					...
					if (hasTop) {
						walkDownPath([currentPos[0] - 1, currentPos[1]]);
					}
					if (hasBottom) {
						walkDownPath([currentPos[0] + 1, currentPos[1]]);
					}
					if (hasLeft) {
						walkDownPath([currentPos[0], currentPos[1] - 1]);
					}
					if (hasRight) {
						walkDownPath([currentPos[0], currentPos[1] + 1]);
					}
	...
	Array.from(summitPerTrailhead.values()).map((r) => {
		score += r.size;
	});
